...
for k = 1:maxIter % begin loop
    ...
    [dvStep, ~, ~, ~, lambda_out] = quadprog(H_quadprog, f_quadprog, A_quadprog, b_quadprog, [], [], [], [], x0_quadprog, qp_options);
    lambda_new = lambda_out.ineqlin;

    % Advice from Dr. Wang - use a small step size along the calculated step direction
    alpha = 0.75; % tune this to provide a suitable solution - should really be a line search   
...
tTotal = toc(tStart); % record the elapsed time since algorithm began
...
disp('=======================================================')
disp('Constrained Optimization Results (SQP):');
disp(['Iterations:', num2str(length(history.iter))]);
disp('Converged Design Variables:');
disp(['b: ', num2str(history.dv_vals(1,end)), ' meters.']);
disp(['h: ', num2str(history.dv_vals(2,end)), ' meters.']);
disp(['t1: ', num2str(history.dv_vals(3,end)), ' meters.']);
disp(['t2: ', num2str(history.dv_vals(4,end)), ' meters.']);
